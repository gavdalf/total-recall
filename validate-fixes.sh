...
if grep -q 'CONTENT=.*jq.*content.*empty' scripts/observer-agent.sh && grep -q 'REASONING=.*jq.*reasoning.*empty' scripts/observer-agent.sh; then
    echo "✅ Empty content handling logic present in observer"
else
    echo "❌ Empty content handling missing in observer"
fi

if grep -q 'CONTENT=.*jq.*content.*empty' scripts/reflector-agent.sh && grep -q 'REASONING=.*jq.*reasoning.*empty' scripts/reflector-agent.sh; then
